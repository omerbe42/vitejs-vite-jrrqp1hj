      ...
      <div className="min-h-screen bg-neutral-950 text-neutral-100 flex flex-col select-none" dir="rtl">
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="w-64 bg-neutral-900/50 border-l border-neutral-800 p-4 space-y-2">
            <button onClick={() => setAdminTab('OVERVIEW')} className={`w-full flex items-center gap-3 p-3 rounded-xl text-sm font-bold transition ${adminTab === 'OVERVIEW' ? 'bg-amber-500 text-neutral-950' : 'text-neutral-400 hover:bg-neutral-800 hover:text-white'}`}><IconMap /> מדדים ושטח</button>
            <button onClick={() => setAdminTab('DOCS')} className={`w-full flex items-center gap-3 p-3 rounded-xl text-sm font-bold transition ${adminTab === 'DOCS' ? 'bg-amber-500 text-neutral-950' : 'text-neutral-400 hover:bg-neutral-800 hover:text-white'}`}><IconFileText /> פנקס מסמכים</button>
            ...
            {adminTab === 'OVERVIEW' && (
              <div className="space-y-6">
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div className="p-5 bg-neutral-900 border border-neutral-800 rounded-2xl"><span className="text-sm text-neutral-400">רוח עגורן 1</span><div className="text-4xl font-black mt-2 text-amber-400">22 <span className="text-lg">קמ"ש</span></div></div>
                  <div className="p-5 bg-neutral-900 border border-neutral-800 rounded-2xl"><span className="text-sm text-neutral-400">בקשות חומרים מקבלנים</span><div className="text-4xl font-black mt-2 text-blue-400">1</div></div>
                </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className="text-[10px] text-neutral-400 font-medium bg-neutral-800 px-1.5 py-0.5 rounded">
              {currentUser.role === 'MANAGER' ? 'סיור שטח (מנהל)' : currentUser.role === 'CRANE' ? 'מנופאי' : currentUser.role === 'SIGNALER' ? 'אתת' : currentUser.role === 'CONTRACTOR' ? 'קבלן' : 'פועל'}
            </span>
          </div>
        </div>
